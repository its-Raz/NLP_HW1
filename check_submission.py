...
def calc_scores(e):
    ...
    for sub in os.listdir(COMP_FILES_PATH):
        ...
        comp1_file = [x for x in os.listdir(
            cur_dir) if x.startswith('comp_m1')]
        comp2_file = [x for x in os.listdir(
            cur_dir) if x.startswith('comp_m2')]
        ...
        if len(comp1_file) != 1:
            ...
            ids = comp1_file[0].replace('comp_m1_', '').split('.')[
                0].split('_')
        ...
        if len(comp2_file) != 1:
            ...
            ids = comp2_file[0].replace('comp_m2_', '').split('.')[
                0].split('_')
...
def open_zip():
    ...
    req_files = [f"report_{ID1}_{ID2}.pdf", "code",
                 "generate_comp_tagged.py"] + comp_files
    for file in req_files:
        if file not in dir_files:
            print(f"{file} is not exists.")
            errors = True
    for file in comp_files:
        shutil.copy(os.path.join(output_directory_path, file),
                    os.path.join(COMP_FILES_PATH, f"{ID1}_{ID2}"))
    return errors
